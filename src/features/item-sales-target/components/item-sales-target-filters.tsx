...
interface ItemSalesTargetFiltersProps {
  search?: string;
  status?: ItemSalesTargetStatus;
}

export function ItemSalesTargetFilters({ search = '', status }: ItemSalesTargetFiltersProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [searchValue, setSearchValue] = useState(search);
  const [statusValue, setStatusValue] = useState<ItemSalesTargetStatus | 'ALL'>(status ?? 'ALL');
    ...
    >
      <div className='grid gap-3 md:grid-cols-2 xl:flex-1'>
        <div className='space-y-2'>
          <Label htmlFor='item-target-search'>Search</Label>
        ...
        <div className='space-y-2'>
          <Label htmlFor='item-target-status'>Status</Label>
          <Select
            value={statusValue}
            onValueChange={(value) => setStatusValue(value as ItemSalesTargetStatus | 'ALL')}
          >
            <SelectTrigger id='item-target-status'>
