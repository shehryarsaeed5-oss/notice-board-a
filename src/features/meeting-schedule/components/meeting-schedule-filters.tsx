...
interface MeetingScheduleFiltersProps {
  search?: string;
  status?: MeetingScheduleStatus;
}

export function MeetingScheduleFilters({ search = '', status }: MeetingScheduleFiltersProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [searchValue, setSearchValue] = useState(search);
  const [statusValue, setStatusValue] = useState<MeetingScheduleStatus | 'ALL'>(status ?? 'ALL');
    ...
    >
      <div className='grid gap-3 md:grid-cols-2 xl:flex-1'>
        <div className='space-y-2'>
          ...
          <div className='relative'>
            ...
            <Input
              ...
              placeholder='Title, location, or organizer'
              className='pl-9'
            />
          </div>
        </div>
        ...
        <div className='space-y-2'>
          ...
          <Select
            value={statusValue}
            onValueChange={(value) => setStatusValue(value as MeetingScheduleStatus | 'ALL')}
          >
