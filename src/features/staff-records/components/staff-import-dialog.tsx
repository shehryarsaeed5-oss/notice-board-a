...
import { importStaffMembersCsv } from '../api/client';
import {
  buildStaffImportTemplateCsv,
  prepareStaffImportRows,
  type StaffImportPreparedResult,
  type StaffImportPreviewRow
} from '../lib/staff-import';

const MAX_CSV_BYTES = 1_000_000;

function createEmptyPreview(): StaffImportPreparedResult {
  return {
    rows: [],
    validRows: [],
    totalRows: 0,
...
function getValidationBadgeClass(isValid: boolean) {
  return isValid
    ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
    : 'border-destructive/30 bg-destructive/10 text-destructive';
}

interface StaffImportPreviewTableRowProps {
  row: StaffImportPreviewRow;
}

function StaffImportPreviewTableRow({ row }: StaffImportPreviewTableRowProps) {
  return (
    <TableRow>
      ...
      <TableCell>{formatCellValue(row.phone)}</TableCell>
      ...
      <TableCell>
        <span className={cn('text-xs font-medium', getValidationBadgeClass(row.isValid))}>
          {row.isValid ? 'Ready' : row.validationMessages.join(' • ')}
        </span>
      </TableCell>
    </TableRow>
  );
...
export function StaffImportDialogTrigger() {
  ...
  const [preview, setPreview] = useState<StaffImportPreparedResult>(() => createEmptyPreview());

  const importMutation = useMutation({
    mutationFn: () => importStaffMembersCsv({ csvText }),
    onSuccess: (result) => {
      const message = `Imported ${result.importedCount} staff records. Skipped ${result.skippedCount} duplicates. Failed ${result.failedCount} rows.`;
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : 'Failed to import staff CSV');
    }
  });

  const hasValidationErrors = useMemo(
    () => preview.rows.some((row) => !row.isValid) || preview.headerErrors.length > 0,
    [preview]
  );
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const prepared = prepareStaffImportRows(text);
      setCsvText(text);
      ...
      if (prepared.headerErrors.length > 0) {
        ...
        setParseError('CSV must contain at least one staff data row.');
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='sm:max-w-6xl'>
        <DialogHeader>
          ...
          <DialogDescription>
            Download the template first, fill in staff details, then upload the completed CSV to
        ...
        <div className='flex max-h-[78vh] flex-col gap-4 overflow-hidden'>
          <div className='grid gap-3 rounded-xl border border-border/60 bg-muted/20 p-4 md:grid-cols-3'>
            ...
            <div className='space-y-1'>
              ...
              <p className='text-xs leading-5 text-muted-foreground'>
                Required columns: name, designation, department. Phone, sort order, and status are
                optional.
              </p>
            </div>
            <div className='space-y-2'>
              <div className='flex flex-wrap gap-2'>
                <Button type='button' variant='secondary' onClick={downloadStaffImportTemplate}>
          ...
          <div className='grid gap-3 md:grid-cols-4'>
            ...
            <div className='rounded-xl border border-border/60 bg-card px-4 py-3'>
              ...
              <div className='mt-1 text-2xl font-semibold text-foreground'>
                {preview.rows.length > 0 && !hasValidationErrors ? 'Yes' : 'No'}
              </div>
            </div>
          </div>
                    ...
                    {preview.rows.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={8} className='text-muted-foreground h-28 text-center'>
                          Upload a CSV file to preview imported staff rows.
                        </TableCell>
                      </TableRow>
                    ) : (
                      preview.rows.map((row) => (
                        <StaffImportPreviewTableRow
                          key={`${row.rowNumber}-${row.name}-${row.designation}`}
                          row={row}
                        />
                      ))
                    )}
