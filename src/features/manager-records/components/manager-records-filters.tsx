...
interface ManagerRecordsFiltersProps {
  search?: string;
  status?: ManagerRecordStatus;
}

export function ManagerRecordsFilters({ search = '', status }: ManagerRecordsFiltersProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [searchValue, setSearchValue] = useState(search);
  const [statusValue, setStatusValue] = useState<ManagerRecordStatus | 'ALL'>(status ?? 'ALL');
    ...
    >
      <div className='grid gap-3 md:grid-cols-2 xl:flex-1'>
        ...
        <div className='space-y-2'>
          ...
          <Select
            value={statusValue}
            onValueChange={(value) => setStatusValue(value as ManagerRecordStatus | 'ALL')}
          >
            <SelectTrigger id='manager-status'>
