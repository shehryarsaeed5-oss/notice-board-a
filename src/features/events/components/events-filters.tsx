...
export function EventsFilters({ search = '', status }: EventsFiltersProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [searchValue, setSearchValue] = useState(search);
  const [statusValue, setStatusValue] = useState<EventRecordStatus | 'ALL'>(status ?? 'ALL');
    ...
    >
      <div className='grid gap-3 md:grid-cols-2 xl:flex-1'>
        <div className='space-y-2'>
          ...
          <div className='relative'>
            ...
            <Input
              ...
              placeholder='Title, client, company, or screen'
              className='pl-9'
            />
          </div>
        </div>
        ...
        <div className='space-y-2'>
          ...
          <Select
            value={statusValue}
            onValueChange={(value) => setStatusValue(value as EventRecordStatus | 'ALL')}
          >
            <SelectTrigger id='events-status'>
